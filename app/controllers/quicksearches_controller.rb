...
class QuicksearchesController < ApplicationController
  def index
    ...
    #  @internships = if !params[:orientation].present? && !params[:semester].present? && !params[:programming_language_ids].present? && !params[:country].present?
    #                   Internship.includes(:company, :semester, :orientation, :programming_languages).where(completed: true).order('internships.created_at DESC')
    #                 else
    #                   @quicksearch.internships(params)
    #                 end
