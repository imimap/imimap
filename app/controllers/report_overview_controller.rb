...
class ReportOverviewController < ApplicationController
  ...
  def index
    ...
    # @internships = @internships.where(companies: { country: countries })
    #  if params[:country].present?
    #  # @internships = @internships.where(students: { last_name: last_names })
    #  if params[:last_name].present?
    #  # @internships = @internships.where(
    #  students: { first_name: first_names })
    #  if params[:first_name].present?
    #  # @internships = @internships.where(semesters: { name: names })
