...
ssh  -i id_rsa_$DEPLOYMENT_ENVIRONMENT -o StrictHostKeyChecking=no $DEPLOMENT_USER@$DEPLOYMENT_HOST "export set TAG=$DEPLOYMENT_TAG; export set SECRET_KEY_BASE=$SECRET_KEY_BASE; export set RAILS_MASTER_KEY=$RAILS_MASTER_KEY; . ./deployment-steps-on-production-machine.sh"
exit_on_error $?
