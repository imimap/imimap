...
sshid=
dumpdir="../imimap-dumps"
prod_dump:
	mkdir -p $(dumpdir)
	ssh deployer@imi-map.f4.htw-berlin.de $(sshid) "docker exec postgresql pg_dump -h localhost -U imi_map  imi_map_production" > $(dumpdir)/imi-map-$(shell date +%Y-%m-%d--%H-%M-%S).pgdump
bash:
