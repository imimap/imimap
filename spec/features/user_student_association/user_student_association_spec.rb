...
describe 'Student login:' do
  describe 'without devise database auth' do
    it 'student object is created if not present - bug' do
      email = 's0150112@htw-berlin.de'
      user = create(:student_user_without_student, email: email)
      user.feature_toggles = %i[
        student_can_edit_internship
        student_can_print_internship_application
      ]
      user.save
      expect do
        sign_in_with_mail(email: email, password: 'foofoofoo123123')
